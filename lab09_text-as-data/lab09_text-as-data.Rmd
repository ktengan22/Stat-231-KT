...
It is important to remove stop words otherwise you'll get a false understanding of the distribution of words. Stop words are obviously far more common than other words so you may have a distribution with 95% of the words being "the." 

```{r top-words-plot2, fig.width = 3.6, fig.height = 2.8}
poems_words_all %>%
  count(word, sort = TRUE) %>% 
  slice(1:10) %>%
  ggplot(aes(x = reorder(word, n), y = n, 
...
tf-idf basically says that if these words occur a bajillion times, they aren't important so it will automatically down-weight them. 
...
The word slick has a slightly positive sentiment with a score of 2. This is interesting because when I first saw the word I had a negative reaction to it. But looking at the definition and thinking of the phrase "you're looking slick today!" means you're looking smooth!
...
Each row in the dataset represents a word in one column and a word for an associated emotion with that word in the other column of that row.
...
##  <!-- 5.3 -->User (and Consumer!) Beware: Do you see any issues in applying these lexicons (developed fairly recently) to the Emily Dickinson poems?
 ...
 Emily Dickinson, growing up in the early to mid 1800s utilized a very different diction than present day. Her words may not be present in our recently developed lexicon dataset. They also may mean something different than they do or are associated with today. 

##  <!-- 5.4 -->The lexicons are based on unigrams.  Do you see any disadvantages of basing the sentiment on single words?

"an n-gram is a contiguous sequence of n items from a given sample of text or speech." from https://en.wikipedia.org/wiki/N-gram 
This is a one word sequence. It may not catch the sentiment of bigrams or chains of texts that go hand in hand. 
...
21% are not scored by the NRC lexicon.

```{r missed-words}
nrc_missed_words <- word_frequencies %>%
  anti_join(nrc_lexicon, by = "word")

7948/37118 #(nrows of nrc missed / nrows of poem_words)

nrow(nrc_missed_words)/nrow(poems_words)
```
...
set.seed(69)
poems_subset <- sample(poems_words$words, size = 10)

top_nrc <- nrc_lexicon %>%
  filter(text %in% poems_subset) %>%
  arrange(desc(sentiment)) %>%
  group_by(words) %>%
  slice(1:10) %>%
  ungroup()

ggplot(data = poem_words, aes(x = reorder(word, poem_words), y = poem_words,
                             fill = text)) +
  geom_col(show.legend = FALSE) +
  facet_wrap(~ title, ncol = 2, scales = "free") +
  coord_flip() +
