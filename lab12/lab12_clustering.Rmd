...
> Amherst and Tufts are the most similar. 
...
2 is the optimal number of clusters at it is the elbow. 
...
Yes the clusters are different if the scales are standardized or unstandardized. 
...
Red cluster has lower admissions rates but higher SAT scores. 
...
set.seed(23)
ccdata_scaled <- ccdata %>% 
  select(ends_with("scaled")) %>% 
  kmeans(centers = 3, nstart = 20)

# Centroids
ccdata_scaled$centers

# Cluster sizes
ccdata_scaled$size

ccdata_scaled_clustered <- ccdata %>%  
 add_column(cluster = factor(ccdata_scaled$cluster))
