...
bern_vec <- rbinom(n = 1000, size = 1, p = 0.85)

ggplot(data.frame(bern_vec), aes(x = bern_vec)) +
  geom_bar(color = "black", fill = "grey") + 
  labs(x = "x",
       title = "Values generated from Binomial distribution",
       subtitle = "Size = 1, p = 0.2")
```
...
When sample size goes up, the distribution becomes more and more normal. 
...
>There will be a set of varying p-values. Not unimodal and not normal because 
they are randomized. Slightly constant. 
...
> What is the probability of falsely rejecting $H_0$ for the first predictor at the $\alpha = 0.05$ significance level (empirical Type I error rate for $X_1$)?  5%
...
> What is the probability that *at least one of the 100 predictors* is statistically significant at the $\alpha = 0.05$ level? Very low.
...
## <!-- 4.1 -->*Note:* The *overall test* means the overall $F$ test testing *$H_0:$ all $\beta_j = 0$*, vs. *$H_a:$ at least one $\beta_j \neq 0$*.  Note that you can extract this p-value for the overall F-test using the code `glance(model)$p.value`, where the `glance()` function is from the **broom** package. For example, if we fit 100 different bivariate models and only the models with X12, X37, and X93 turned out to have significant p-values, then we would do the following:

```{r}
test_model <- lm(y ~ X12 + X37 + X93, data = dat)
summary(test_model)
...
## <!-- 4.2 -->Before beginning to code, think about the last `for` loop created as a starting point.  What steps need to be added/modified in that simulation to carry out this simulation?



## <!-- 4.3 -->Write code to walk through each of the general steps for one iteration (i.e., outside of a `for` loop). *Hint:* For a single vector of p-values, the function `which()` can be used to identify the indices of values that are less than 0.05 (e.g., `which(pvalues < 0.05)` might return "16, 49, 58, ..."). We can then use `paste0()` to piece together predictor names (e.g., to get `"X16"`, `"X49"`, ...), and another layer of `paste(..., collapse = " + ")` to combine all the predictor names into the right-hand side of the linear model formula (e.g., to get `"X16 + X49 + ..."`).
