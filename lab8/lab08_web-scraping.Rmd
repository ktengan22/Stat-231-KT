...
We need to get the source code and url. Assign a url to an object then read the html. Then go through the elements and isolate all of the html source code to be displayed in hte future. 
...
```{r}
url <- "https://en.wikipedia.org/wiki/List_of_Emily_Dickinson_poems" #scrape url

paths_allowed(url) #path allowed
```
...
> There are patterns, but some exceptions include dashes, commas, or exclamation
points. Everything is just separated by underscores. Probably better to 
scrape the links directly. 
...
url_table <- tibble(title = poem_table, href = url_links)
```
