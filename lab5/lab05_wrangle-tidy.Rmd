...
```{r}
# Create a data frame with state info (combine all the state info from the dataset)
...
```{r}
coverage <- coverage[-c(1, 10),]

spending <- spending[-c(1, 10),]
```
...
- mutate and log transform `spending` and `employer covereage` 
- lable the axes and define the color of the years
- find the R to see if there's a relationship or look at the graph and fit a line maybe 
...
```{r}
joined <- coverage %>% 
  inner_join(spending, by = c("Location" = "Location"))
...
joined_long <- joined %>% 
  pivot_longer(-Location, #pivot everything else but location
              names_to = c("Year", "Category"),
              names_sep = "__", #separate the data into two columns "year" and "category" that is separated by "__"
              values_to = "Amount")

# joined_long

joined_wider <- joined_long %>% 
  pivot_wider(
    names_from = Category,
    values_from = Amount,
    #values_fill = 0
