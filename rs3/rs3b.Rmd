...
1. Save the output that you build to `output$` - the output be accessible to the ui object when you call output.
2. Build the output with the `render*()`- helps assemble into html for final appearance and keeps track of the reactions and the reactivity.
3. Access input values with `input$` - 
Thus, shiny will create reactivity by using inputs to build rendered outputs.

Three other components to build a shiny app are a ui component, a function defined for the server, and a component that can knit the ui component and server function together. The ui component builds the web document the app lives in. The function contains the instructions needed to build plots and do clustering. Finally, the knitting component knits the ui and the server function together using the shiny app. 
...
Whenever the user changes the value on a slide, it tells the render function 
that this value is out of date. The render function then reacts and updates 
to the new value, and displays that updated graphical output. This happens
instantaneous to the user, so as the user changes the values on the slide, 
the graph also changes and updates. The reactive output will automatically
...
If the reactive value is set to number of observations and the render displayes
a randomized histogram. When the user changes the number of randomized values
on the slide, the rendered histogram will also change to reflect the new
numbers. 
...
I am having extreme difficulty with isolating the reactive title change.
With what I have now, whenever I change the title for the histogram,
both the histogram plot and the title react. Even without moving the slider,
when I change the name, the values and plot also change. When I tried to isolate
the title, the title no longer reacted and changed. How can I isolate the title?
