...
```{r, fig.align = "center", fig.height = 3.5}
migration_igraph2 <- graph_from_data_frame(migration_easteur,
           directed = TRUE)

migration_network2 <- ggnetwork(migration_igraph2)
head(migration_network2)

ggplot(data = migration_network2, aes(x = x, y = y, 
          xend = xend, yend = yend)) +
 geom_edges(arrow = arrow(type = "closed", length = unit(8, "pt")),
    color = "lightgray") +
 geom_nodes() +
...
```{r, fig.align = "center", fig.height = 3.5}
ggplot(data = migration_network2, 
    aes(x = x, y = y, 
      xend = xend, yend = yend)) +
...
I think the more effective one is the edge width through ti can get cluttered at times. I feel that with the color it is hard to see the distinction between the various edges. 
...
Based on the degree, I feel like Poland is the most central to the network. We see that there were a total of 18 migrations, 9 out and 9 in. 


```{r}
igraph::degree(migration_igraph2) 

igraph::degree(migration_igraph2, mode = "out")

igraph::degree(migration_igraph2, mode = "in") 
```
...
After considering the weighted degrees, we see that UKR has the greatest number for total movement. We would not have been able to know that if we had just looked at degrees. 

```{r}
# Get edge weights 
migration_edge_weights <- edge_attr(migration_igraph2, name = "Y2000")

# Total movement
strength(migration_igraph2, weights = migration_edge_weights)

# Total movement out
strength(migration_igraph2, weights = migration_edge_weights, mode = "out") 

# Total movement in
strength(migration_igraph2, weights = migration_edge_weights, mode = "in")
```
...
There are 107 nodes (vertices) and 352 edges in this network
...
> Note: The number of *possible* edges in an undirected graph is 

${V \choose 2} = \frac{V(V-1)}{2}$.

The edge density of this network is 0.06207. When we calculate the number by hand using the equation of possible edges, we get the same number. 

```{r}
edge_density(got_igraph)
...
`is_connected()` returns "TRUE" meaning the graph is connected. We can see this by using our eyes. All the nodes are connected to edges. All the characters are seen on the graph. 
...
The diameter of the network is 6 meaning that the greatest distance between any pair of vertices in the network is 6 units. 
...
The five characters with the highest degree are Tyrion, Jon, Robb, Sansa, and Jaime. The five characters with the highest weighted degree are Tyrion, Jon, Sansa, Jaime, and Bran. 
...
Robb may have a higher degree but lower weighted degree because he may have more total edges, yet they may not be as weighted as Bran. This is just like the question we did above using degree and strength. 
...
Yes it does match both figure 2 and 3. 
