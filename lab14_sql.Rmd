...
Codebook, Measurements, Measurements pre 2020, PI info, PI info old, subjects
...
variable character, integers, floats. 1,349,667 observations in Measurements 
and 3,147 observations in Subjects.
...
Block 2 would not work because tidyverse functions are not applicable to SQL. 
...
The SQL chunk just works directly in the chunk
...
If you group before you use WHERE vs if you group after you use HAVING.
...
```{sql, connection = con, output.var = "Hearing"}
SELECT  Frequency,
        Absorbance,
        EarStatus
FROM Measurements
WHERE Idenitifier = Rosowski_2012
```
