...
> There are 142 observations for each variable.
...
> The correlation coefficient between x and y is -0.06. This is essentially a very weak (insignificant), slightly negative relationship. 
...
> It is incredibly fascinating that the scatterplot of x and y form an outline of a t-rex. There is no strong linear relationship. Looking at the points as a whole, it is not homoscedastic. 
...
> r is -0.0629. This barely differs from the other dino dataset. However, the scatter plot differs a lot. 

```{r star-cor}
# Start with `datasaurus_dozen`
# Filter for observations where `dataset == "dino"`
# Store the resulting filtered data frame as a new data frame called `dino_data`
dino_data2 <- datasaurus_dozen %>%
  filter(dataset == "star")
...
> Very cool! No, the points on the scatter plot are now in the shape of a star.

```{r star-plot, eval = TRUE, fig.width = 2.5, fig.height = 2}
ggplot(data = dino_data2, mapping = aes(x = x, y = y)) +
  geom_point()
```
...
```{r plots-cors, eval = TRUE, fig.width = 2.5, fig.height = 4}
...
> I would argue that the correlation coefficient is not an appropriate summary of these datasets. Looking at the table, the r is essentially the exact same for every single dataset. However, visually we see that they differ absolutely significantly by shape and design. A better indicator would be looking at the scatterplots or performing other tests to check the significance of the x and y relationship. 


## <!-- 3.2 -->What do you think `ncol = 3` does in the code above? What about `digits = 3`?

> I think that ncol = 3 refers to making 3 columns in the output. We see this visually. I would take a guess and say that digits = 3 presents the correlation coefficients values up to 3 places after the decimal point. Every values goes to the thousandths place. 
...
```{r test-chunk-options, echo = FALSE}
